...
bot.command("todo", async (ctx) => {
  ...
  const inlineKeyboard = [
    [{ text: "Done ✅", callback_data: "done-0" }],
  ];
...
bot.action("done-0", async (ctx) => {
  const newKeyboard = [
    [{ text: "Undone 🔄", callback_data: `undone-0` }],
  ];
  ...
  if ("text" in targetMessage) {
    ...
    await ctx.editMessageText(newMessageText, { parse_mode: 'HTML', reply_markup: { inline_keyboard: newKeyboard } });
  }
})
...
bot.action("undone-0", async (ctx) => {
  ...
  if ("text" in targetMessage) {
    await ctx.editMessageText("все еще не сделано", { parse_mode: 'HTML', reply_markup: { inline_keyboard: newKeyboard } });
  }
})
